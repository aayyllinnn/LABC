...
function podzielMapeNaPuzzle(imageDataUrl, mapWidth, mapHeight) {
    ...
    img.onload = () => {
        const pieceWidth = mapWidth / 4;
        const pieceHeight = mapHeight / 4;
...
function sprawdzCzyUkładankaZłożona() {
    ...
    if (complete) {
        ...
        console.log('Puzzle nie są jeszcze w pełni zmontowane lub są zmontowane nieprawidłowo.');
    }
}
...
function wyswietlPowiadomienie(tresc) {
    if (Notification.permission === 'granted') {
        ...
        alert(tresc); // Zapasowy alert, jeśli powiadomienia są zablokowane
